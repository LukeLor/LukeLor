...
local GetBackdoorTime = function(rawString)
...
, {"Drones incoming!", "Hide, HIde, HIDE!!!", "Get out of their way!", "Drones on break.", "Drones passing through!"},
	{"Okay, okay, don't look, alright?", "Alma's here...", "Don't- look, Alma..."},
	{"Bash-ing through the snow...", "Bash is queued up!", "Bash is here, beware.", "I think you should hide..? Bash is probably on its way."}
,
}

	local deathmsgs = {{"Sorry I failed you..."},{"..."},{"Are you there!?"}, {"Please, respond!"}, {"What... happened?"},{"I'll do better, I promise..."}}

			local sayDeath = coroutine.create(function()
		while task.wait(2) do 
			if game.Players.LocalPlayer.PlayerGui.MainUI.Statistics.Visible == true then
	...
	for i = 1, #text do
		...
		SpeakerIconHide()
				repeat task.wait(2) until char:WaitForChild("Humanoid").Health < 0
				end
	end
end)
...
local text = entitytablelines[14][math.random(1,3)]
		...
		SpeakerIconHide()
			...
			if inst.Name == "Alma" then
				local text = entitytablelines[19][math.random(1,3)]
		print(text)
		SpeakerIconShow("99087926706059")
		for i = 1, #text do
		...
		SpeakerIconHide()
			...
			if inst.Name == "BashMoving" then 
				local text = entitytablelines[20][math.random(1,4)]
		print(text)
		SpeakerIconShow("99087926706059")
		for i = 1, #text do
