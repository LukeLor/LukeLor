...
local function Roll(NewNum:number, Label:TextLabel) 
    local rollc = coroutine.create(function() 
        ...
        clonedlabel.Parent = Ui.MainFrame 
...
game.ReplicatedStorage.GameData.LatestRoom:GetPropertyChangedSignal("Value"):Connect(function() 
    ...
    if #stringed_num == 3 then 
        Roll(string.sub(stringed_num,3,3), Ui.MainFrame.FourthDigit) 
        if string.sub(stringed_num,2,2) ~= sh then 
            Roll(string.sub(stringed_num,2,2), Ui.MainFrame.ThirdDigit) 
        ...
        if string.sub(stringed_num,1,1) ~= th then 
            Roll(string.sub(stringed_num,1,1), Ui.MainFrame.SecondDigit) 
    ...
    if #stringed_num == 4 then 
        Roll(string.sub(stringed_num,4,4), Ui.MainFrame.FourthDigit) 
        if string.sub(stringed_num,3,3) ~= sh then 
            Roll(string.sub(stringed_num,3,3), Ui.MainFrame.ThirdDigit) 
        ...
        if string.sub(stringed_num,2,2) ~= th then 
            Roll(string.sub(stringed_num,2,2), Ui.MainFrame.SecondDigit) 
        ...
        if string.sub(stringed_num,1,1) ~= fh then 
            Roll(string.sub(stringed_num,1,1), Ui.MainFrame.FirstDigit) 
    ...
    if #stringed_num == 5 then 
        Roll("N", Ui.MainFrame.FirstDigit) 
        Roll("/", Ui.MainFrame.SecondDigit) 
        Roll("A", Ui.MainFrame.ThirdDigit) 
        Roll(".", Ui.MainFrame.FourthDigit) 
    end 
end)
